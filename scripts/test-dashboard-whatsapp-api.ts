...
async function testDashboardWhatsAppAPI() {
  ...
  const testCases = [
    ...
    {
      name: 'Message with additional metadata',
      payload: {
  ...
  try {
    ...
    console.log(`✅ Correctly rejected invalid payload: ${error.response?.data?.error || error.message}`);
  }
  ...
  try {
    ...
    if (error.response?.status === 405) {
      ...
      console.log(`⚠️  Unexpected response for GET method: ${error.response?.status} - ${error.message}`);
    }
  }
